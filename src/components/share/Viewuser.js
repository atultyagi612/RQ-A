...
const Viewuser = (props) => {
    ...
    const getlikes=async()=>{
        const snapshot = await getCountFromServer(query(usercollectionref, where('user',"==",props.item.email) ))
        setlikes(snapshot.data().count)
    }
    const getcontribution =async()=>{
        const snapshot = await getCountFromServer(query(usercollectionref2, where('email',"==",props.item.email) ))
...
<div data-mdb-toggle="modal" data-mdb-target="#exampleModaluserview" style={{cursor:"pointer"}}>
  <h5 style={{borderRadius:"5px"}} onMouseOver={(e)=>{console.log(e.target.style.background="#f8f8f8")}} onMouseOut={(e)=>{console.log(e.target.style.background="white")}}>
    <img src={props.item.photourl} className="rounded-circle shadow-1-strong me-3"
            width="30"
            height="30" alt="Avatar" loading="lazy"  />  {props.item.name}</h5></div>
  ...
  <div className="container py-5 h-100">
    ...
    padding: "13px"}}></button>
          <div className="card-body p-4">
            <div className="d-flex text-black viewuser">
              ...
              <div className="flex-grow-1 ms-3" style={{    overflow: "overlay"}}>
                ...
                <p className="mb-2 pb-1" style={{color: "#2b2a2a"}}>{props.item.email}</p>
